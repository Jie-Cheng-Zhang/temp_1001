...
from utility.text_sentiment import get_word_level_sentiment
...
def train(multimodal_model, loader, optimizer, criterion, scheduler, device, encoder_model):
    ...
    for iter, data in enumerate(loader):
        ...
        loss = (criterion(predictions, label.float()) * 0.85 + loss_S * 0.075 + loss_F * 0.075) * 15
...
def evaluate(args, epoch, best_acc, multimodal_model, loader, criterion, device, encoder_model):
    epoch_loss = 0
    ...
    print(
        'Epoch: {}/{}:  Macro F1:  {} Macro Precision: {}  Macro Recall: {}  Binary F1: {}  Binary Precision: {}  Binary Recall: {}  Acc: {}  Best Acc: {}'.format(
            epoch, args.epoch, macro_f1, macro_precision, macro_recall, binary_f1, binary_precision, binary_recall, acc,
            best_acc
        ))
...
def run(args):
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        test_loss, test_acc = evaluate(args, epoch, best_test_acc, multimodal_model, test_loader, criterion, device, encoder_model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--multimodal_lr', type=float, default=3e-4)
    parser.add_argument('--multimodal_weight_decay', type=float, default=1e-2)
